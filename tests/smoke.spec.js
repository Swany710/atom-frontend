...
test.describe('App boot', () => {
    ...
    test('text input is present', async ({ page }) => {
        await page.goto(BASE_URL);
        const input = page.locator('#mainTextInput');
...
test.describe('Text send', () => {
    test('typing and pressing Enter adds user message', async ({ page }) => {
        ...
        const userMsg = page.locator('.user-message, [data-role="user"]').first();
    ...
    test('send button is disabled while request is in-flight', async ({ page }) => {
        await page.goto(BASE_URL);
        const input  = page.locator('#mainTextInput');
        const sendBtn = page.locator('#mainSendButton');
...
test.describe('Knowledge base panel', () => {
    test('KB panel loads and shows search UI', async ({ page }) => {
        ...
        await expect(page.locator('#kbSearchInput')).toBeVisible();
    });
});
...
test.describe('Scheduled tasks panel', () => {
    test('tasks panel opens and shows task filters', async ({ page }) => {
        ...
        await expect(page.locator('#tasksPendingBtn')).toBeVisible();
        await expect(page.locator('#tasksAllBtn')).toBeVisible();
    });
});
